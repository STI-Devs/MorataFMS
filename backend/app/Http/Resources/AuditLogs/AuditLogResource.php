...
class AuditLogResource extends JsonResource
{
     ...
     * Each branch only accesses columns that exist on the matched model type.
     */
    private function resolveAuditableLabel(): ?string
    {
        ...
        if ($model) {
            $label = match (true) {
                $model instanceof User, $model instanceof Client => $model->name,
                $model instanceof ImportTransaction => $model->customs_ref_no,
                $model instanceof ExportTransaction => $model->bl_no,
                $model instanceof Document => $model->filename,
                $model instanceof TransactionRemark => "Remark #{$model->id}",
                default => null,
            };
        ...
        // Snapshot keys mirror the live column names.
        $old = $this->old_values;
        if (is_array($old)) {
            return $old['name']
                ?? $old['filename']
