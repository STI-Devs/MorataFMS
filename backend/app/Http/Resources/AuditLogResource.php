...
class AuditLogResource extends JsonResource
{
    ...
    private function resolveAuditableLabel(): ?string
    {
        ...
        if ($model) {
            ...
            if ($model instanceof \App\Models\TransactionRemark) {
    ...
    private function formatValues(?array $values): ?array
    {
        ...
        foreach ($values as $key => $value) {
            ...
            if (in_array($key, ['author_id', 'resolved_by', 'assigned_user_id', 'user_id', 'uploaded_by']) && $value) {
                ...
                if (!$type && $this->auditable_type === \App\Models\TransactionRemark::class) {
                    $remark = \App\Models\TransactionRemark::find($this->auditable_id);
                    if ($remark) {
    ...
    private function resolveTransactionRef($type, $id)
    {
        ...
        if (!isset(self::$transactionCache[$cacheKey])) {
            $model = $type::find($id);
            if ($model instanceof \App\Models\ImportTransaction) {
                $ref = $model->customs_ref_no;
            } elseif ($model instanceof \App\Models\ExportTransaction) {
                $ref = $model->bl_no;
            } else {
