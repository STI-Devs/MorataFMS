...
const successData = {
    ...
    workloads: [
        { id: 1, name: 'Sarah Velasco', role: 'Senior Encoder', active: 12, overdue: 2 },
        { id: 2, name: 'Mike Tan', role: 'Encoder', active: 9, overdue: 1 },
    ],
    records_summary: {
        in_review_count: 8,
        completed_count: 5,
...
describe('AdminDashboard', () => {
    beforeEach(() => {
        mockUseAdminDashboard.mockReturnValue({ data: successData, isLoading: false, isError: false });
    });
    ...
    it('renders the brokerage dashboard sections and full quick-action labels', () => {
        ...
        // Monthly Volume bar chart + Live Status Mix donut render ChartContainers.
        expect(document.querySelectorAll('[data-chart]')).toHaveLength(2);
    });
    ...
    it('shows a skeleton while the dashboard is loading', () => {
        mockUseAdminDashboard.mockReturnValue({ data: undefined, isLoading: true, isError: false });
        ...
        expect(screen.queryByText('Operation Queue')).not.toBeInTheDocument();
    });

    it('shows an error state and retries when the query fails', () => {
        const refetch = vi.fn();
        mockUseAdminDashboard.mockReturnValue({ data: undefined, isLoading: false, isError: true, refetch });
