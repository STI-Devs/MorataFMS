...
export const ProcessorTransactionPage = () => {
    ...
    const renderImportQueue = () => {
        ...
        const readyTransactions = data.filter((transaction) => {
            const actionability = getImportProcessorActionability(transaction.stages);

            return actionability.ppa || actionability.port_charges;
        });
                    ...
                    transactions={waitingTransactions.map((transaction) => {
                        ...
                        return (
                            <TransactionQueueCard
                                ...
                                meta={`ETA: ${transaction.arrival_date || '—'}`}
                                tone="waiting"
                                blocker={getImportProcessorWaitingReason(stages)}
    ...
    const renderExportQueue = () => {
        ...
        const readyTransactions = data.filter((transaction) => {
            const actionability = getExportProcessorActionability(transaction.stages);

            return actionability.cil || actionability.dccci;
        });
        const waitingTransactions = data.filter((transaction) => ! readyTransactions.includes(transaction));
                    ...
                    transactions={waitingTransactions.map((transaction) => {
                        ...
                        return (
                            <TransactionQueueCard
                                ...
                                meta={`Vessel: ${transaction.vessel || '—'}`}
                                tone="waiting"
                                blocker={getExportProcessorWaitingReason(stages)}
                                agingLabel={getWaitingAgeLabel(transaction.waiting_since ?? transaction.created_at)}
                                actionLabel="Monitor Progress"
                                onAction={() => setSelectedTx({
                                    id: transaction.id,
    ...
    return (
        <div className="flex h-full flex-1 flex-col">
            <div className="flex flex-col px-4 pt-4 pb-2">
                ...
                <p className="mt-1 text-sm text-text-secondary">View shared pending processor stages across active transactions and upload the required files.</p>
            </div>
            <div className="flex gap-4 border-b border-border px-4 mt-2">
                ...
                >
                    ...
                    {importsQuery.data && importsQuery.data.length > 0 && (
                        <span className="rounded-full bg-blue-100 dark:bg-blue-900/40 px-1.5 py-0.5 text-[10px] font-bold leading-none text-blue-600 dark:text-blue-400">
...
const DocumentStatusBadge = ({
    ...
    if (status === 'completed') {
        ...
        colorClass = 'bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800/50';
    }
