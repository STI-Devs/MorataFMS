...
// ΓöÇΓöÇΓöÇ Year accordion sub-folder row ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
export const SubFolderRow = ({ groupKey, docs, yr, filterStatus, nav, openMenuKey, setOpenMenuKey, onViewHistory }: SubFolderRowProps) => {
    ...
    return (
        <div key={groupKey}
            ...
            style={{ gridTemplateColumns: '28px 1fr 70px 80px 80px 110px 130px 100px' }}>
            ...
            <span className="text-xs text-text-secondary tabular-nums">{docs.length.toLocaleString()}</span>
            <span className="text-xs text-text-secondary tabular-nums">{blMap.size.toLocaleString()}</span>

            <span className={`text-xs font-bold tabular-nums ${folderPct >= 90 ? 'text-emerald-500' : folderPct >= 50 ? 'text-amber-500' : 'text-red-500'
                }`}>{folderPct}%</span>

            <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full border w-fit ${statusCls}`}>
                <span className={`w-1.5 h-1.5 rounded-full ${statusLabel === 'Complete' ? 'bg-emerald-500' : statusLabel === 'Partial' ? 'bg-amber-400' : 'bg-red-500'
                    }`} />
                {statusLabel}
            </span>

            <span className="text-xs text-text-muted truncate">{lastUpd}</span>
...
// ΓöÇΓöÇΓöÇ Year accordion row ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
const YearRow = ({ yr, isOpen, toggleYear, filterType, filterStatus, nav, openMenuKey, setOpenMenuKey, onViewHistory }: YearRowProps) => {
    ...
    return (
        <div className="border-b border-border last:border-b-0">
            <div
                className={`flex items-center gap-3 px-5 py-3.5 border-b border-border cursor-pointer select-none transition-colors ${isOpen ? 'bg-blue-500/8 border-blue-500/20' : 'bg-surface-secondary hover:bg-hover'}`}
                ...
                <span className={`text-base font-black ${isOpen ? 'text-blue-500' : 'text-text-primary'}`}>FY {yr.year}</span>
            ...
            {isOpen && (
                <div>
                    <div className="grid items-center gap-3 px-5 py-2.5 border-b border-t border-border bg-surface-secondary sticky top-0 z-10"
                        style={{ gridTemplateColumns: '28px 1fr 70px 80px 80px 110px 130px 100px' }}>
...
// ΓöÇΓöÇΓöÇ Main FolderView export ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
