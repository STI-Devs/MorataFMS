...
export const ArchiveDocumentRow = ({ doc, onDelete }: Props) => {
    ...
    return (
        <div className="grid items-center gap-4 px-4 py-3 border-b border-border/40 hover:bg-hover transition-colors"
            style={{ gridTemplateColumns: '28px 1fr 1.4fr 80px 28px 48px' }}>

            {/* File ext icon */}
            <div className="w-7 h-7 rounded-md flex items-center justify-center shrink-0"
                style={{ backgroundColor: c.bg }}>
                <svg className="w-3.5 h-3.5" fill="none" stroke={c.color} viewBox="0 0 24 24">
            ...
            {/* Stage label */}
            <p className="text-xs text-text-secondary truncate">{stage}</p>
            ...
            <div className="flex items-center justify-end gap-0.5">
                ...
                <button
                    ...
                    title="Delete" onClick={e => { e.stopPropagation(); onDelete(doc.id); }}>
                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
